...
def train(
    ...
    for i in range(len(train_list)):
        ...
        (
            ...
            bag_features_deep,
            A_feat,
        ) = milnet(bag_feats, bag_feats_deep, training="yes")

        loss_class = criterion(bag_prediction, bag_label) + criterion(
            bag_prediction_deep, bag_label
        )
        ...
        sys.stdout.write(
            "\r Training bag [%d/%d] bag loss: %.4f" % (i, len(train_list), loss.item())
        )
    return (
        total_loss / len(train_list),
        total_loss_mse / len(train_list),
        total_loss_attnreg / len(train_list),
    )
...
def test(
    ...
    with torch.no_grad():
        for i in range(len(test_list)):
            ...
            (
                ...
                bag_features_deep,
                A_feat,
            ) = milnet(bag_feats, bag_feats_deep)
    ...
    for i in range(0, len(test_list)):
        ...
        bag_score_classwise[test_labels[i]] += np.array_equal(
            test_labels[i], test_predictions[i]
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Train ABMIL on 5x patch features learned by encoder"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--weight_decay", default=5e-2, type=float, help="Weight decay [5e-3]"
    )

    # path for PathExpert features
    parser.add_argument(
        "--dataset_split_path", default="", type=str, help="Dataset folder name"
    )
    parser.add_argument(
        "--dataset_split_path_test", default="", type=str, help="Dataset folder name"
    )
    parser.add_argument(
        "--features_path", default="", type=str, help="Dataset folder name"
    )

    # path for Deep features
    parser.add_argument(
        "--dataset_split_deep_path", default="", type=str, help="Dataset folder name"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--features_deep_path", default="", type=str, help="Dataset folder name"
    )
    parser.add_argument(
        "--features_deep_path_test", default="", type=str, help="Dataset folder name"
    )

    parser.add_argument("--save_path", default="", type=str, help="Dataset folder name")

    parser.add_argument(
        "--bins", default=10, type=int, help="Number of bins for binarization"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dropout_patch", default=0.0, type=float, help="Patch dropout rate [0]"
    )
    parser.add_argument(
        "--dropout_node",
        default=0.0,
        type=float,
        help="Bag classifier dropout rate [0]",
    )
    parser.add_argument(
        "--organ", default="lung", type=str, help="for grouping in wandb"
    )
    parser.add_argument("--torch_seed", default=-1, type=int, help="")
    parser.add_argument(
        "--feat_type", default="", type=str, help="for grouping in wandb"
    )
    ...
    parser.add_argument(
        ...
        default="yes",
        type=str,
        help=" no, yes. for grouping in wandb",
    )
    parser.add_argument(
        "--stop_gradient",
        default="no",
    ...
    b_classifier = mil.BClassifier(
        ...
        input_size_deep=feats_size_deep,
    ...
    save_path += (
        ...
        + str(args.lr).split(".")[-1]
        + "_wd0p"
        + str(args.weight_decay).split(".")[-1]
        + "_dropout0p"
        + str(args.dropout_patch).split(".")[-1]
    )
    ...
    for i in bags_path:  # from train and val both
        train_index_select_list.extend(
            list(np.arange(dataset_split_dict[i][0], dataset_split_dict[i][1]))
        )

    print(features_array.shape, features_array[train_index_select_list].shape)

    for i in range(features_array.shape[1]):
        non_minusone_indices = np.where(features_array[:, i] != -1)[
            0
        ]  # -1 when feature is None i.e. can't be calculated
        non_minusone_indices_in_train = np.intersect1d(
            train_index_select_list, non_minusone_indices
        )

        features_array[non_minusone_indices, i] = (
            features_array[non_minusone_indices, i]
            - features_array[non_minusone_indices_in_train, i].mean()
        ) / (features_array[non_minusone_indices_in_train, i].std() + 1e-6)
    ...
    for epoch in range(1, args.num_epochs):
        ...
        train_loss_bag, train_loss_bag_mse, train_loss_bag_attnreg = train(
            train_path,
            dataset_split_dict,
            dataset_split_deep_dict,
            features_array,
        ...
        val_loss_bag, avg_score_val, aucs_val, bag_score_classwise_val = val(
            val_path,
            dataset_split_dict,
            dataset_split_deep_dict,
            features_array,
        ...
        test_loss_bag, avg_score, aucs, bag_score_classwise = test(
            test_path,
            dataset_split_dict_test,
            dataset_split_deep_dict_test,
            features_array,
            features_deep_array_test,
            milnet,
            criterion,
            feats_size,
            feats_size_deep,
        ...
        print(
            "\r Epoch [%d/%d] train loss: %.4f val loss: %.4f, average score: %.4f, AUC: %.4f"
            % (
                epoch,
                args.num_epochs,
                train_loss_bag,
        ...
        print(
            "\r test loss: %.4f, average score: %.4f, AUC: %.4f"
        ...
        wandb.log(
            {
                "train loss": train_loss_bag,
                "train loss attnreg": train_loss_bag_attnreg,
                "train loss mse": train_loss_bag_mse,
                "val loss": val_loss_bag,
                "test loss": test_loss_bag,
                "val score": avg_score_val,
                "test score": avg_score,
            },
            step=epoch,
        )
        ...
        for class_index in range(args.num_classes):
            wandb.log(
                {
                    "Validation AUC Class " + str(class_index): aucs_val,
                    "Test AUC Class " + str(class_index): aucs,
                    "Validation score Class "
                    + str(class_index): bag_score_classwise_val[class_index],
                    "Test score Class "
                    + str(class_index): bag_score_classwise[class_index],
                },
                step=epoch,
            )
