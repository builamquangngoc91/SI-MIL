...
parser.add_argument(
    "--feat_path", type=str, default="test_dataset/Handcrafted_features"
)
parser.add_argument(
    "--save_path", type=str, default="test_dataset/Handcrafted_features"
)
parser.add_argument(
    "--train_test_dict_path", type=str, default="test_dataset/train_test_dict.json"
)
...
parser.add_argument(
    "--remove_noneoplastic", type=str, default="False"
)  # put it 'True' for Lung cancer since it doesn't contains noneoplastic cell type
...
print(
    "shape of features_csv",
    features_csv.shape,
    "shape of features_csv training patches",
    features_csv.iloc[train_index_select_list].shape,
)
...
for i, col in tqdm(enumerate(features_csv.columns)):
    try:
        _, intervals = pd.qcut(
            features_csv.iloc[train_index_select_list, i],
            bins,
            labels=False,
            retbins=True,
            duplicates="drop",
        ...
        normalized_df[col] = pd.cut(
            features_csv[col], bins=intervals, labels=False, include_lowest=True
        )
...
low_cell_patches = np.intersect1d(
    np.where(normalized_df["number of neoplastic cells"] == 0)[0],
    np.where(normalized_df["number of inflamatory cells"] == 0)[0],
)

low_cell_patches = np.intersect1d(
    low_cell_patches, np.where(normalized_df["number of connective cells"] == 0)[0]
)
low_cell_patches = np.intersect1d(
    low_cell_patches, np.where(normalized_df["number of necrosis cells"] == 0)[0]
)

if remove_noneoplastic == "False":
    low_cell_patches = np.intersect1d(
        low_cell_patches,
        np.where(normalized_df["number of no-neoplastic cells"] == 0)[0],
    )

print(
    "low_cell_patches ",
    low_cell_patches.shape,
    " cell_percentage ",
    cell_percentage.shape,
    "back_percentage ",
    back_percentage.shape,
)
print(
    "np.intersect1d(cell_percentage, back_percentage).shape",
    np.intersect1d(cell_percentage, back_percentage).shape,
)

remove_low_cell_patches = np.intersect1d(
    np.intersect1d(cell_percentage, back_percentage), low_cell_patches
)

filter_indices = np.setdiff1d(
    np.intersect1d(cell_percentage, back_percentage), remove_low_cell_patches
)
...
for i in bags_path:
    train_index_select_list.extend(
        list(
            np.arange(
                dataset_split_dict_train_filtered_temp[i][0],
                dataset_split_dict_train_filtered_temp[i][1],
            )
        )
    )
...
if norm_feat == "max":
    ...
    normalized_df[~np.isnan(features_csv)] = (features_csv[~np.isnan(features_csv)]) / (
        train_max + 1e-6
    )
    ...
    normalized_df = features_csv.copy()
    # Normalize the non-missing values using mean and std
    normalized_df[~np.isnan(features_csv)] = (
        features_csv[~np.isnan(features_csv)] - train_mean
    ) / (train_std + 1e-6)
    normalized_df = normalized_df.fillna(-1)
    ...
    for i, col in tqdm(enumerate(features_csv.columns)):
        try:
            ...
            non_nan_indices = np.where(np.array(~np.isnan(features_csv[col])) == True)[
                0
            ]
...
for i in range(1, cell_parse):
    ...
    for cell_feat in [
        ...
        for stats in ["mean", "std", "skew", "kurtosis"]:
            ...
            cell_stat_column_name.append(
                labeldict[i] + " cells: " + stats + " of their " + cell_feat
            )
...
for rad in radii[1:]:
    athena_column_name.append(
        "neoplastic" + " cells: k-function at radius " + str(int(rad)) + " pixels"
    )
...
if len(list_filtered) == 0:
    print(
        "Warning: No patches passed the filtering criteria. Creating empty output files."
    )
    # Create empty output files
    with open(save_path + "/train_dict.pickle", "wb") as f:
        pickle.dump({}, f)
    with open(save_path + "/test_dict.pickle", "wb") as f:
        pickle.dump({}, f)
    exit(0)
...
for i in dataset_split_dict_train_filtered:
    dataset_split_list_train_filtered_deep.extend(
        list_filtered[
            dataset_split_dict_train_filtered[i][0] : dataset_split_dict_train_filtered[
                i
            ][1]
        ]
    )
    ...
    c += (
        dataset_split_dict_train_filtered[i][1]
        - dataset_split_dict_train_filtered[i][0]
    )

    dataset_split_dict_train_filtered_deep[i].append(c)

    dataset_split_dict_train_filtered_deep[i].append(
        dataset_split_dict_train_filtered[i][-1]
    )
...
for i in dataset_split_dict_test_filtered:
    dataset_split_list_test_filtered_deep.extend(
        list_filtered[
            dataset_split_dict_test_filtered[i][0] : dataset_split_dict_test_filtered[
                i
            ][1]
        ]
    )
    ...
    dataset_split_dict_test_filtered_deep[i].append(
        dataset_split_dict_test_filtered[i][-1]
    )
