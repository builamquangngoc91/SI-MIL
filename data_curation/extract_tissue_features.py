...
parser.add_argument(
    "--save_path",
    type=str,
    default="test_dataset/Handcrafted_features/tissue_statistics",
)
parser.add_argument(
    "--hovernet_json_path", type=str, default="test_dataset/Hovernet_output/json"
)
...
def single_crop_features(
    ...
    wsi_crop_patch = slide.read_region(
        (start_x_point, start_y_point),
        0,
        (stop_x_point - start_x_point, stop_y_point - start_y_point),
    )
    ...
    background_mask = np.where(
        background_mask < background_threshold, background_mask, 0
    )
    ...
    wsi_crop_patch_segmentation = Image.new(
        "RGB", (stop_x_point - start_x_point, stop_y_point - start_y_point)
    )
    ...
    wsi_crop_patch_segmentation = np.where(
        wsi_crop_patch_segmentation > 0, 1, 0
    ).astype(np.uint8)
    ...
    cell_percent = (
        wsi_crop_patch_segmentation
    ).sum() / wsi_crop_patch_segmentation.size
...
def run_extraction(file_name):
    ...
    if not os.path.isfile(save_file_path):
        ...
        image_patches_list = np.array(all_list)[
            all_dict[file_name[:-1]][0] : all_dict[file_name[:-1]][1]
        ]
        ...
        start_coordinate_list = (
            np.array(start_coordinate_list, dtype=object) // mag_ratio
        )
