...
parser.add_argument(
    "--save_path", type=str, default="test_dataset/Handcrafted_features/cell_statistics"
)
...
def single_crop_features(
    ...
    for i in range(1, 1 + no_of_cell_types):
        if np.where(points_type == i)[0].shape[0] == 0:
            cell_statistics_class_wise.extend(
                [None] * property_list.shape[1]
            )  # if no cell of i'th type present, then put zeros mean
            cell_statistics_class_wise.extend(
                [None] * property_list.shape[1]
            )  # if no cell of i'th type present, then put zeros std
            cell_statistics_class_wise.extend(
                [None] * property_list.shape[1]
            )  # if no cell of i'th type present, then put zeros skew

            cell_statistics_class_wise.extend(
                [None] * property_list.shape[1]
            )  # if no cell of i'th type present, then put zeros kurtosis
        ...
        cell_statistics_class_wise.extend(
            kurtosis(per_class_properties, axis=0)
        )  # kurtosis
