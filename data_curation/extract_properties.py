...
parser.add_argument(
    "--json_path", type=str, default="test_dataset/Hovernet_output/json"
)
            ...
            try:
                ...
                if len(list(pred_data["nuc"].keys())) == 0:
                    ...
                    for keys in tqdm(list(pred_data["nuc"].keys())):
                        ...
                        temp_contour = (
                            np.array(pred_data["nuc"][keys]["contour"]).copy()
                            // mag_ratio
                        )
                        ...
                        wsi_cell_crop = slide.read_region(
                        ...
                        cell_property = regionprops_table(
                            cell_image,
                            intensity_image=wsi_cell_crop,
                            properties=properties_list,
                        )

                        temp_prop_list = (
                            np.array(list(cell_property.values())).reshape(-1).tolist()
                        )

                        wsi_cell_crop = (rgb2gray(wsi_cell_crop) * 255).astype(np.uint8)

                        # intensity features
                        wsi_cell_crop_masked = wsi_cell_crop[
                            np.where(cell_image == 1)[0], np.where(cell_image == 1)[1]
                        ].copy()
                        ...
                        # texture features
                        glcm = graycomatrix(
                            wsi_cell_crop * cell_image,
                            distances=[1],
                            angles=[0],
                            levels=256,
                        )

                        temp_prop_list.extend(
                            [
                        ...
                        prop_dict[keys] = {}
                        prop_dict[keys]["centroid"] = (
                            np.array(pred_data["nuc"][keys]["centroid"]) // mag_ratio
                        )
