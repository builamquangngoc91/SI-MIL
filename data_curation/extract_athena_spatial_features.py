...
parser.add_argument(
    "--cell_properties_path", type=str, default="test_dataset/cell_property"
)
parser.add_argument("--list_dict_path", type=str, default="test_dataset/patches")
parser.add_argument(
    "--save_path",
    type=str,
    default="test_dataset/Handcrafted_features/athena_statistics",
)
...
def single_crop_features(
    ...
    parts = patch_name.split("/")[-1].rsplit(".", 1)[0].split("_")
    column, row = parts[-2], parts[-1]
    ...
    for point_class in range(1, no_of_cell_types + 1):
        ...
        if (
            len(points[points_type == point_class]) > 100
        ):  # if less points then not good enough k-function
    ...
    for i in range(len(points)):
        ...
        local_points_types = [
            points_type[j] for j in local_points_ids
        ]  # Assuming you have a cell_types list
        probabilities = np.unique(local_points_types, return_counts=True)[1] / len(
            local_points_ids
        )
    ...
    richness_local_hist = np.array(
        [Counter(richness_per_cell)[j] for j in range(1, 1 + no_of_cell_types)]
    ).tolist()
    shannon_local_hist = np.histogram(
        shannon_per_cell,
        bins=5,
        range=(
            0,
            -(1 / no_of_cell_types * np.log2(1 / no_of_cell_types)) * no_of_cell_types,
        ),
    )[0].tolist()
    simpson_local_hist = np.histogram(
        simpson_per_cell,
        bins=5,
        range=((np.array([1 / no_of_cell_types] * no_of_cell_types) ** 2).sum(), 1),
    )[0].tolist()
    renyi_local_infi_hist = np.histogram(
        renyi_local_infi_per_cell, bins=5, range=(0, -math.log(1 / no_of_cell_types, 2))
    )[0].tolist()
    ...
    infiltration_score_list = [None] * (
        (no_of_cell_types - 1) * 2
    )  # -1 represents no cell type i-i interaction # only tumor with all other infiltration vice-versa

    number_dict = {}
    for i in range(1, no_of_cell_types + 1):
        for j in range(i, no_of_cell_types + 1):
            number_dict[i, j] = A[np.where(points_type == i)[0]][
                :, np.where(points_type == j)[0]
            ].sum()
            if i != j:
    ...
    if number_dict[tumor_id, tumor_id] == 0:
        ...
        for j in range(1, no_of_cell_types + 1):
            if tumor_id == j:
                continue
            infiltration_score_list[counter] = (
                number_dict[tumor_id, j] / number_dict[tumor_id, tumor_id]
            )
            counter += 1
...
def run_extraction(cell_pickle_path):
    ...
    if not os.path.isfile(save_path + "/" + cell_pickle_path):
        ...
        image_patches_list = np.array(all_list)[
            all_dict[cell_pickle_path[:-7]][0] : all_dict[cell_pickle_path[:-7]][1]
        ]
        ...
        for patch_name in tqdm(image_patches_list):
            final_feature_dict[patch_name] = single_crop_features(
                key_list,
                cell_centroid_list,
                type_list,
                property_list,
